...
/**
 * Fungsi untuk mengirim status proses HL7Server_php ke semua klien monitor
 * @returns {Promise<string>} - Promise yang menghasilkan status proses
 */
function kirimSemuaProses() {
  processName = "HL7Server_php";
  return new Promise((resolve, reject) => {
    pm2.connect((err) => {
      if (err) {
        console.error(err);
  ...
  ws.on("message", (message) => {
    ...
    switch (perintah) {
      ...
      case "list_c":
        ...
        break;
      case "status":
        getPM2ProcessStatus("HL7Server_php")
          .then((status) => {
            console.log(` :-> Status of HL7Server_php: ${status}`);
            ws.send(`${status}`);
          })
          .catch((err) => {
            ws.send(`${status}`);
            console.error(`:-> Error getting status of HL7Server_php: ${err}`);
          });
        break;
      case "restart":
        restartPM2Process("HL7Server_php");
        ws.send(status_report);
        break;
      case "start":
        startPM2Process("HL7Server_php");
        ws.send(status_report);
        break;
      case "monitor":
      ...
      case "lspam":
        spam.add(ws);
        ws.send("Anda akan menerima laporan spam");
        break;
